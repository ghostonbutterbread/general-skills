...
class ScriptManagerPolicyTests(unittest.TestCase):
    ...
    def test_minimal_records_and_maintenance_boundary(self) -> None:
        ...
        self.assertIn("bounded discovery-metadata edits", self.text)
        self.assertIn("does not authorize unrelated skill or policy changes", self.text)
        self.assertIn("does not override its restriction", self.text)
