...
- Example:
  ```bash
  python3 ~/.hermes/synced-skills/accounts/gmail-otp/scripts/gmail_otp.py otp \
    --query 'is:unread newer_than:2d' --max 10
  ```
- Tests: `python3 -m py_compile skills/accounts/gmail-otp/scripts/gmail_otp.py`; mocked send allowlist and read-only static capability checks.
- Owner: general-skills/accounts/gmail-otp.
